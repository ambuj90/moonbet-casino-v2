...
/* ================= GLASS STYLES ================= */
const glassBase =
  "trust-btn3 border-2 border-white/40 backdrop-blur-[67.5px]";
...
const TagChip = ({ iconSrc, text, onRemove }) => (
  <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs text-white ${glassBase} ${glassInactive}`}>
    {iconSrc && <img src={iconSrc} className="w-3.5 h-3.5" />}
    {text}
    <span className="ml-1 opacity-70" onClick={onRemove} style={{ cursor: 'pointer' }}>×</span>
  </div>
);
...
const QuickPick = ({ iconSrc, label, active, onClick }) => (
  <button onClick={onClick} className={`h-[56px] rounded-xl flex flex-col items-center justify-center gap-1 text-[11px] ${glassBase} ${active ? glassActive : glassInactive}`}>
...
const GameTypeChip = ({ label, checked, onClick }) => (
  <button onClick={onClick} className={`min-w-[120px] px-4 py-2 rounded-xl flex items-center gap-2 text-sm ${glassBase} ${checked ? glassActive : glassInactive}`}>
    <span className={`w-4 h-4 rounded border flex items-center justify-center ${checked ? "bg-white" : "border-white/40"}`}>
      {checked && (
...
const ThemeChip = ({ label, active, onClick }) => (
  <button onClick={onClick} className={`px-4 py-2 rounded-full text-sm ${glassBase} ${active ? glassActive : glassInactive} ${active ? "text-white" : "text-[#C9CBFF]"}`}>
    {label}
  </button>
);
...
const ProviderBtn = ({ text, active, onClick }) => (
  <button onClick={onClick} className={`px-4 py-2 rounded-xl flex items-center gap-2 text-xs ${glassBase} ${active ? glassActive : glassInactive} ${active ? "text-white" : "text-[#C9CBFF]"}`}>
    <span className={`w-3.5 h-3.5 rounded border ${active ? "bg-white" : "border-white/40"}`} />
    {text}
  </button>
);
...
const FiltersPanel = ({ open, onClose, onApplyFilters }) => {
  ...
  const getAllSelectedTags = () => {
    ...
    selectedQuickPicks.forEach(id => {
      ...
      if (item) {
        tags.push({ type: 'quickPick', id, label: item.label, icon: item.icon });
      }
    });
  ...
  const getTotalSelectedCount = () => {
    return selectedQuickPicks.length + selectedGameTypes.length + selectedThemes.length + selectedProviders.length;
  };
  ...
  const visibleGameTypes = showAllGameTypes ? GAME_TYPES : GAME_TYPES.slice(0, 6);
        ...
        <>
          ...
          >
            ...
            <div className=" mt-16 h-[64px] px-5 flex items-center justify-between border-b border-white/20">
              ...
              <button onClick={onClose} className="w-9 h-9 rounded-lg border border-white/30 bg-white/15 text-white">
                ✕
              </button>
            </div>
            ...
            <div className="px-5 py-4 flex flex-wrap gap-2">
              {selectedTags.length === 0 ? (
                <span className="text-[#8E90C9] text-xs">No filters selected</span>
              ) : (
...
<div>
  ...
  <div
    ref={sliderRef}
    className={`relative h-10 flex items-center ${glassBase} ${glassInactive} rounded-xl px-3`}
  >
    {/* TRACK */}
    ...
    <div
      ...
      style={{ left: `calc(${volatility}% - 12px)` }}
      onMouseDown={(e) => {
        e.preventDefault();
    ...
    >
      <img
        src="/icons/volatility-thumb.png"  // 🔥 YOUR ICON HERE
        className="w-6 h-6"
        draggable={false}
      />
    </div>
  ...
  <div className="flex justify-between text-[11px] text-[#C9CBFF] mt-2 px-1">
    ...
    <span>Wild</span>
              ...
              <div>
                ...
                {GAME_TYPES.length > 6 && (
                  <p className="text-sm text-[#8E90C9] mt-3" onClick={() => setShowAllGameTypes(!showAllGameTypes)} style={{ cursor: 'pointer' }}>
              ...
              <div>
                ...
                {THEMES.length > 9 && (
                  <p className="text-sm text-[#8E90C9] mt-3" onClick={() => setShowAllThemes(!showAllThemes)} style={{ cursor: 'pointer' }}>
              ...
              <div>
                ...
                {PROVIDERS.length > 6 && (
                  <p className="text-sm text-[#8E90C9] mt-3" onClick={() => setShowAllProviders(!showAllProviders)} style={{ cursor: 'pointer' }}>
            ...
            <div className="px-5 py-4 border-t border-white/10 flex gap-3">
              <button onClick={handleApplyFilters} className="flex-1 py-3 rounded-xl text-sm font-semibold text-white bg-gradient-to-b from-[#FFB27A] to-[#D64A1F]">
                Show {getTotalSelectedCount() > 0 ? `${getTotalSelectedCount()} Filters` : "128 Games"}
              </button>
              <button onClick={clearAllFilters} className="flex-1 py-3 rounded-xl bg-[#6C6F90] text-white text-sm">
                Clear All
              </button>
            </div>
          </motion.div>
